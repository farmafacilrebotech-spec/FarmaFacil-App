-- =============================================================================
-- 025_pharmacy_membership_reinvite_v1.sql
-- Amplía ff_transition_pharmacy_membership_v1 con la acción `reinvite`:
--   revoked → invited
--
-- Reutiliza la misma membership (UNIQUE pharmacy_id+profile_id).
-- NO crea auth.users ni profiles. NO pasa a active.
-- NO toca memberships de otras farmacias.
--
...
BEGIN
  ...
  ELSIF v_action = 'reinvite' THEN
    IF v_status <> 'revoked' THEN
      RAISE EXCEPTION 'invalid_state'
        USING ERRCODE = 'P0001',
              MESSAGE = 'Solo se puede volver a invitar a un usuario con acceso revocado.';
    END IF;

    -- Misma fila: revoked → invited. No active directo. No otra membership.
    UPDATE public.pharmacy_memberships m
    SET
      status = 'invited',
      invited_at = v_now,
...
COMMENT ON FUNCTION public.ff_transition_pharmacy_membership_v1(uuid, uuid, text) IS
  'Transiciones membership: suspend|reactivate|revoke|cancel_invitation|reinvite. Valida pharmacy_id+membership_id, autoriza platform.users.write o memberships.manage, bloquea último PHARMACY_OWNER activo. reinvite: revoked→invited. Sin hard-delete ni auth.users.';
...
-- DOWN (desarrollo):
-- Volver a desplegar el cuerpo de 023_pharmacy_membership_lifecycle_v1.sql
-- (sin acción reinvite).
